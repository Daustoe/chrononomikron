use super::{bsp_dungeon::BspDungeonBuilder, RoomSorter, RoomSort, EndingPosition, XEnd, YEnd, 
     room_draw::RoomDrawer, room_exploder::RoomExploder, 
    corridors::NearestCorridors,  StartingPosition, 
    BuilderChain, BuilderMap, CellularAutomataBuilder, CullUnreachable, DLABuilder, DistantExit, 
    DrunkardsWalkBuilder, MetaMapBuilder, PrefabBuilder, TileType, XStart, YStart};
...
pub fn limestone_cavern_builder(new_depth: i32, width: i32, height: i32) -> BuilderChain {
    ...
    chain.with(StartingPosition::new(XStart::LEFT, YStart::CENTER));
...
pub fn limestone_deep_cavern_builder(new_depth: i32, width: i32, height: i32) -> BuilderChain {
    ...
    chain.with(StartingPosition::new(XStart::LEFT, YStart::TOP));
    //chain.with(VoronoiSpawning::new());
    chain.with(DistantExit::new());
    chain.with(CaveDecorator::new());
...
pub fn limestone_transition_builder(new_depth: i32, width: i32, height: i32) -> BuilderChain {
    ...
    chain.with(StartingPosition::new(XStart::LEFT, YStart::CENTER));
    chain.with(CullUnreachable::new());
    chain.with(EndingPosition::new(XEnd::RIGHT, YEnd::CENTER));
    chain
}
